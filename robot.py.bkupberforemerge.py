    ...
    def grasp(self, position, heightmap_rotation_angle, workspace_limits):
        ...
        if self.is_sim:
            ...
            while True:
                ...
                if (tool_analog_input2 < 3.7 and
                        (abs(new_tool_analog_input2 - tool_analog_input2) <
                         0.01) and
                        all([np.abs(actual_tool_pose[j] - position[j]) <
                             self.tool_pose_tolerance[j] for j in range(3)])) or \
