...
HOST = "10.75.15.91"            # The Robot IP address
    ...
    if 1 == 1:
        if program_run == 0:
            try:
                ...
                # Print the receive data in \x hex format (notice that data above 32 hex will be represented by the ascii code e.g. 50 will show P
                print "Received data as x format = ", repr(msg)
                print ""
                # Convert the data from \x hex format to plain hex
                msg = msg.encode("hex")
                ...
                # get rid of the 0b and fill with zeros so byte take 4 positions
                byte_1_bin_in = byte_1_bin_in[2:].zfill(4)
                ...
                # get rid of the 0b and fill with zeros so byte take 4 positions
                byte_2_bin_in = byte_2_bin_in[2:].zfill(4)
                ...
                # get rid of the 0b and fill with zeros so byte take 4 positions
                byte_1_bin_out = byte_1_bin_out[2:].zfill(4)
                ...
                # get rid of the 0b and fill with zeros so byte take 4 positions
                byte_2_bin_out = byte_2_bin_out[2:].zfill(4)
                ...
                # Give time to read the read result before change output
                time.sleep(10.00)
