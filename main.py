        ...
        while True:
            if nonlocal_variables['executing_action']:
                ...
                if not grasp_only:
                    ...
                    if explore_actions:
                        print('Strategy: explore (exploration probability: %f)' % (
                            explore_prob))
                        nonlocal_variables['primitive_action'] = 'push' if np.random.randint(
                            0, 2) == 0 else 'grasp'
                    else:
                ...
                if heuristic_bootstrap and nonlocal_variables['primitive_action'] == 'push' and no_change_count[0] >= 2:
                    ...
                    nonlocal_variables['best_pix_ind'] = trainer.push_heuristic(
                    ...
                    nonlocal_variables['best_pix_ind'] = trainer.grasp_heuristic(
                        valid_depth_heightmap)
                    no_change_count[1] = 0
                ...
                if nonlocal_variables['primitive_action'] == 'push':
                    ...
                    local_region = valid_depth_heightmap[max(best_pix_y - safe_kernel_width, 0):min(best_pix_y + safe_kernel_width + 1, valid_depth_heightmap.shape[0]), max(
                        best_pix_x - safe_kernel_width, 0):min(best_pix_x + safe_kernel_width + 1, valid_depth_heightmap.shape[1])]
                ...
                if nonlocal_variables['primitive_action'] == 'push':
                    trainer.executed_action_log.append(
                        [0, nonlocal_variables['best_pix_ind'][0], nonlocal_variables['best_pix_ind'][1], nonlocal_variables['best_pix_ind'][2]])  # 0 - push
                elif nonlocal_variables['primitive_action'] == 'grasp':
                    trainer.executed_action_log.append(
        ...
        if 'prev_color_img' in locals():
            ...
            if experience_replay and not is_testing:
                ...
                if sample_ind.size > 0:
                    ...
                    new_sample_label_value, _ = trainer.get_label_value(sample_primitive_action, sample_push_success, sample_grasp_success, sample_change_detected,
...
if __name__ == '__main__':
    ...
    parser.add_argument('--is_mock', dest='is_mock', action='store_true', default=False, help='mock run?')
